...
public class SynthesisTableMenu extends AbstractContainerMenu {
    ...
    public SynthesisTableMenu(int id, Inventory playerInv, SynthesisTableBlockEntity entity) {
        ...
        // Memory Card Slot (Slot 82 - Limite de 1 para gravação segura)
    ...
    public boolean clickMenuButton(@NonNull Player player, int buttonId) {
        if (buttonId == 0) {
            ...
            this.blockEntity.setAutomationMode(0); // Craft Mode
            return true;
        } else if (buttonId == 2) {
            this.blockEntity.setAutomationMode(1); // Encoder Mode
            return true;
        } else if (buttonId == 3) {
    ...
    public @NonNull ItemStack quickMoveStack(@NonNull Player player, int index) {
        ...
        if (slot.hasItem() && slot.isActive()) {
            ...
            if (index < 83) { // Inside table inventory (0..82)
                if (!this.moveItemStackTo(stackInSlot, 83, this.slots.size(), true)) {
                    return ItemStack.EMPTY;
                }
            } else { // Inside player inventory (83..118)
                if (blockEntity.isAutomationActive() && stackInSlot.is(ModItems.MEMORY_CARD.get())) {
                    if (!this.moveItemStackTo(stackInSlot, 82, 83, false)) {
                        if (!this.moveItemStackTo(stackInSlot, 0, 81, false)) return ItemStack.EMPTY;
                    }
                } else {
                    if (!this.moveItemStackTo(stackInSlot, 0, 81, false)) {
                        return ItemStack.EMPTY;
                    }
                }
            }
